...
@implementation RBNetworkAgent {
    AFHTTPSessionManager *_manager;
    RBNetworkConfig *_config;
    AFJSONResponseSerializer *_jsonResponseSerializer;
    AFXMLParserResponseSerializer *_xmlParserResponseSerialzier;
    NSMutableDictionary<NSNumber *, RBBaseRequest *> *_requestsRecord;
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _config = [RBNetworkConfig sharedConfig];
...
- (NSString *)buildRequestUrl:(RBBaseRequest *)request {
    ...
    for (id<RBUrlFilterProtocol> f in filters) {
...
- (void)cancelAllRequests {
    ...
    if (allKeys && allKeys.count > 0) {
        ...
        for (NSNumber *key in copiedKeys) {
            Lock();
            RBBaseRequest *request = _requestsRecord[key];
            Unlock();
...
- (BOOL)validateResult:(RBBaseRequest *)request error:(NSError * _Nullable __autoreleasing *)error {
...
- (void)handleRequestResult:(NSURLSessionTask *)task responseObject:(id)responseObject error:(NSError *)error {
    Lock();
    RBBaseRequest *request = _requestsRecord[@(task.taskIdentifier)];
    Unlock();
...
- (void)requestDidSucceedWithRequest:(RBBaseRequest *)request {
    @autoreleasepool {
...
- (void)requestDidFailWithRequest:(RBBaseRequest *)request error:(NSError *)error {
    request.error = error;
    ...
    if ([request.responseObject isKindOfClass:[NSURL class]]) {
        ...
        if (url.isFileURL && [[NSFileManager defaultManager] fileExistsAtPath:url.path]) {
            ...
            request.responseString = [[NSString alloc] initWithData:request.responseData encoding:[RBNetworkUtils stringEncodingWithRequest:request]];
...
- (void)addRequestToRecord:(RBBaseRequest *)request {
    Lock();
    _requestsRecord[@(request.requestTask.taskIdentifier)] = request;
    Unlock();
}

- (void)removeRequestFromRecord:(RBBaseRequest *)request {
    Lock();
...
- (NSURLSessionDownloadTask *)downloadTaskWithDownloadPath:(NSString *)downloadPath
    ...
    BOOL resumeDataIsValid = [RBNetworkUtils validateResumeData:data];
...
- (NSURL *)incompleteDownloadTempPathForDownloadPath:(NSString *)downloadPath {
    ...
    NSString *md5URLString = [RBNetworkUtils md5StringFromString:downloadPath];
