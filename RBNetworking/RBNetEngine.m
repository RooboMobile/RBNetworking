...
- (void)sendRequest:(RBNetRequest *)request completionHandler:(RBCompletionHandler)completionHandler {
    if (request.requestType ==RBRequestNormal) {
        [self xm_dataTaskWithRequest:request completionHandler:completionHandler];
    } else if (request.requestType ==RBRequestUpload) {
        [self xm_uploadTaskWithRequest:request completionHandler:completionHandler];
    } else if (request.requestType ==RBRequestDownload) {
...
- (RBNetRequest *)cancelRequestByIdentifier:(NSString *)identifier {
    ...
    __block RBNetRequest *request = nil;
    if (tasks.count > 0) {
...
- (RBNetRequest *)getRequestByIdentifier:(NSString *)identifier {
    if (identifier.length == 0) return nil;
...
- (void)xm_dataTaskWithRequest:(RBNetRequest *)request
             completionHandler:(RBCompletionHandler)completionHandler {
...
- (void)xm_uploadTaskWithRequest:(RBNetRequest *)request
               completionHandler:(RBCompletionHandler)completionHandler {
    ...
    NSMutableURLRequest *urlRequest = [requestSerializer multipartFormRequestWithMethod:@"POST"
                                                              ...
                                                              constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
                                                                  [request.uploadFormDatas enumerateObjectsUsingBlock:^(RBUploadFormData *obj, NSUInteger idx, BOOL *stop) {
                                                                      if (obj.fileData) {
...
- (void)xm_downloadTaskWithRequest:(RBNetRequest *)request
                 completionHandler:(RBCompletionHandler)completionHandler {
...
- (void)xm_processURLRequest:(NSMutableURLRequest *)urlRequest byXMRequest:(RBNetRequest *)request {
...
- (void)xm_setIdentifierForReqeust:(RBNetRequest *)request
...
- (AFHTTPSessionManager *)xm_getSessionManager:(RBNetRequest *)request {
...
- (AFHTTPRequestSerializer *)xm_getRequestSerializer:(RBNetRequest *)request {
    if (request.requestSerializerType ==RBRequestSerializerRAW) {
        return self.afHTTPRequestSerializer;
    } else if(request.requestSerializerType ==RBRequestSerializerJSON) {
        return self.afJSONRequestSerializer;
    } else if (request.requestSerializerType ==RBRequestSerializerPlist) {
...
- (AFHTTPResponseSerializer *)xm_getResponseSerializer:(RBNetRequest *)request {
    if (request.responseSerializerType ==RBResponseSerializerRAW) {
        return self.afHTTPResponseSerializer;
    } else if (request.responseSerializerType ==RBResponseSerializerJSON) {
        return self.afJSONResponseSerializer;
    } else if (request.responseSerializerType ==RBResponseSerializerPlist) {
        return self.afPListResponseSerializer;
    } else if (request.responseSerializerType ==RBResponseSerializerXML) {
