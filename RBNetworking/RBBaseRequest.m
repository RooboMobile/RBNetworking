...
- (BOOL)loadCacheWithError:(NSError * _Nullable __autoreleasing *)error {
    ...
    if ([self cacheTimeInSeconds] < 0) {
        if (error) {
            *error = [NSError errorWithDomain:RBRequestCacheErrorDomain code:RBRequestCacheErrorInvalidCacheTime userInfo:@{ NSLocalizedDescriptionKey:@"Invalid cache time"}];
        }
        return NO;
    }
    ...
    if (![self loadCacheMetadata]) {
        if (error) {
            *error = [NSError errorWithDomain:RBRequestCacheErrorDomain code:RBRequestCacheErrorInvalidMetadata userInfo:@{ NSLocalizedDescriptionKey:@"Invalid metadata. Cache may not exist"}];
        }
        return NO;
    }
    ...
    if (![self loadCacheData]) {
        if (error) {
            *error = [NSError errorWithDomain:RBRequestCacheErrorDomain code:RBRequestCacheErrorInvalidCacheData userInfo:@{ NSLocalizedDescriptionKey:@"Invalid cache data"}];
        }
        return NO;
    }
...
- (BOOL)validateCacheWithError:(NSError * _Nullable __autoreleasing *)error {
    ...
    if (duration < 0 || duration > [self cacheTimeInSeconds]) {
        if (error) {
            *error = [NSError errorWithDomain:RBRequestCacheErrorDomain code:RBRequestCacheErrorExpired userInfo:@{ NSLocalizedDescriptionKey:@"Cache expired"}];
        }
        return NO;
    }
    // Version
    ...
    if (cacheVersionFileContent != [self cacheVersion]) {
        if (error) {
            *error = [NSError errorWithDomain:RBRequestCacheErrorDomain code:RBRequestCacheErrorVersionMismatch userInfo:@{ NSLocalizedDescriptionKey:@"Cache version mismatch"}];
        }
        return NO;
    }
    // Sensitive data
    ...
    if (sensitiveDataString || currentSensitiveDataString) {
        ...
        if (sensitiveDataString.length != currentSensitiveDataString.length || ![sensitiveDataString isEqualToString:currentSensitiveDataString]) {
            if (error) {
                *error = [NSError errorWithDomain:RBRequestCacheErrorDomain code:RBRequestCacheErrorSensitiveDataMismatch userInfo:@{ NSLocalizedDescriptionKey:@"Cache sensitive data mismatch"}];
            }
            return NO;
        }
    }
    ...
    NSString *currentAppVersionString = [RBNetworkUtils appVersionString];
    if (appVersionString || currentAppVersionString) {
        if (appVersionString.length != currentAppVersionString.length || ![appVersionString isEqualToString:currentAppVersionString]) {
            if (error) {
                *error = [NSError errorWithDomain:RBRequestCacheErrorDomain code:RBRequestCacheErrorAppVersionMismatch userInfo:@{ NSLocalizedDescriptionKey:@"App version mismatch"}];
            }
            return NO;
        }
    }
    ...
    if ([self cacheTimeInSeconds] > 0 && ![self isDataFromCache]) {
        if (data != nil) {
            @try {
                ...
                metadata.appVersionString = [RBNetworkUtils appVersionString];
...
- (void)createBaseDirectoryAtPath:(NSString *)path {
    ...
    if (error) {
        ...
        [RBNetworkUtils addDoNotBackupAttribute:path];
    }
}
...
- (NSString *)cacheBasePath {
    ...
    NSArray<id<RBCacheDirPathFilterProtocol>> *filters = [[RBNetworkConfig sharedConfig] cacheDirPathFilters];
    if (filters.count > 0) {
        for (id<RBCacheDirPathFilterProtocol> f in filters) {
...
- (NSString *)cacheFileName {
    ...
    NSString *baseUrl = [RBNetworkConfig sharedConfig].baseUrl;
    id argument = [self cacheFileNameFilterForRequestArgument:[self requestArgument]];
    NSString *requestInfo = [NSString stringWithFormat:@"Method:%ld Host:%@ Url:%@ Argument:%@",
                             (long)[self requestMethod], baseUrl, requestUrl, argument];
    NSString *cacheFileName = [RBNetworkUtils md5StringFromString:requestInfo];
